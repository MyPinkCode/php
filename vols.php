      ...
      <div class="row tm-content-row">
        <div class="col-sm-12 col-md-12 col-lg-8 col-xl-8 tm-block-col">
          ...
          <div class="tm-bg-primary-dark tm-block tm-block-products">
            <form method="POST" action="create_vol.php" class="tm-login-form">
            <table class="table table-hover tm-table-small tm-product-table">
              ...
              <td style="width:140px"><select class="custom-select form-control" name="no_escale" class="form-control validate" required>
                <option value="0">escale</option>
                <?php
                $conn=connect();	
                ...
                echo
                '
                <option value="'. $row['no_escale'] .'">'. $row['no_escale'] .' : '. $row['heure_depart'] .' --> '. $row['heure_arrivee'] .'</option>';
               }} else {echo '<option value="1">none</option>';} } ?> </select></td>
                  <td><input name="jour" class="form-control validate" required placeholder="jour" type="text" /></td>
                  <td><input name="heure_depart" class="form-control validate" required placeholder="date depart" type="text" /></td>
                  <td><input name="heure_arrivee" class="form-control validate" required placeholder="date arrive" type="text" /></td>
                
                  </tbody>
            </table>
                <button type="submit" name="ajout" class="btn btn-primary btn-block text-uppercase" style="margin-bottom:20px">
              Ajouter nouveau vol
            </button>
                
             
            </form>
            <form method="POST" action="create_vol.php">
              ...
              <table class="table table-hover tm-table-small tm-product-table">
                <thead>
                  <tr>
                    ...
                    <th scope="col" colspan="2">N° vol</th>
                ...
                if($conn){ 
                  ...
                  if ($result->rowCount() > 0) {
                    while($row = $result->fetch()) {
                echo
                  ' <form method="post" action="edit&delete_vol.php" class="tm-login-form"><tr>
                  <td colspan="2" class="tm-product-name">'. $row['no_vol'] .'<input type="text"  name="no_vol" hidden class="form-control" value="'. $row['no_vol'] .'"></td>
                    <td><input type="text"  name="no_aeroport" class="form-control validate" required value="'. $row['no_aeroport'] .'"></td>
                    <td><input type="text"  name="no_escale" class="form-control validate" required value="'. $row['no_escale'] .'"></td>
                    <td><input type="text"  name="jour" class="form-control validate" required value="'. $row['jour'] .'"></td>
